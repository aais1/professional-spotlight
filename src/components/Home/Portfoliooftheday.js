...
export default function PortfolioOfTheWeek({ card }) {
  ...
  return (
      <div className=" w-full max-w-4xl  mx-auto p-1 md:p-4 font-serif">
        ...
        <div className="line-clamp-3" dangerouslySetInnerHTML={{ __html: card.about }}></div>
        <span className="bg-[#124e66]  text-white rounded-md hover:underline text-xs sm:text-base mt-4 inline-block "> <Link to={`/portfolio/${slug}`} className="inline-block px-4 py-1">Read More</Link> </span>
      </div>
  );
}